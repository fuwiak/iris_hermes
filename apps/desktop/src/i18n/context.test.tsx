...
describe('I18nProvider', () => {
  ...
  it('normalizes an initial locale alias and switches translations', async () => {
    render(
      <I18nProvider configClient={null} initialLocale="ru-RU">
  ...
  it('falls back to default locale when config loading fails', async () => {
    const configClient: I18nConfigClient = {
    ...
    render(
      <I18nProvider configClient={configClient} initialLocale="ru">
        <LanguageProbe />
      </I18nProvider>
    )
  ...
  it('reads latest config before saving language and preserves unrelated values', async () => {
    ...
    expect(saveConfig).toHaveBeenCalledWith({
      display: { language: 'ru', skin: 'slate' },
      terminal: { cwd: '/new' }
    })
  })
  ...
  it('keeps LTR document direction for supported locales', async () => {
    render(
