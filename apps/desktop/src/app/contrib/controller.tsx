...
export function ContribController() {
  ...
  const shellSize = embed ? 'h-full min-h-0 w-full' : 'h-screen min-h-0 w-screen'

  return (
    <SidebarProvider
      className={`${embed ? 'h-full' : 'h-screen'} min-h-0 flex-col bg-background`}
    ...
    >
      <ContribWiring>
        <div
          className={`flex ${shellSize} flex-col bg-(--ui-bg-chrome) text-(--ui-text-primary)`}
