...
class TestSmartDeniedPrompt:
    ...
    def test_smart_deny_uses_locale_specific_once_deny_choices(self, monkeypatch, capsys):
        ...
        rendered = capsys.readouterr().out
        assert result == "once"
        assert "[o]один раз" in rendered
        assert "[d]отклонить" in rendered
        assert i18n.t("approval.choose_short", lang="ru").split("|")[1].strip() not in rendered
        assert "o/D" in prompts[0]
